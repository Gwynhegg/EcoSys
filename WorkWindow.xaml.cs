...
namespace EcoSys
{
    ...
    public partial class WorkWindow : Window
    {
        ...
        private Entities.ScenarioEntity scenarios;
        private Entities.ModelEntity model;
        private readonly bool auto_launch;
        ...
        private bool alreadyExist<T>()      //Проверка существования с использованием обобщенного типа
        {
            foreach (object grid in main_grid.Children)     //Пробегаем по всем дочерним элементам главного окна
            {
                ...
                {
                    if (grid is T)
                        ((Grids.IGrid)grid).showGrid();
                    else
                        ((Grids.IGrid)grid).hideGrid();     //Если грид найден, то показываем его, скрывая остальные. Работает даже в случаях, когда грида еще нет
                }
            }

            if (main_grid.Children.OfType<T>().Any())
                return true;
            else
                return false;       //говорим о необходимости создавать новый грид
        }
